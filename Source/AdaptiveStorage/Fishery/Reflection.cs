...
public static class Reflection
{
	...
	{
		public const SystemBindingFlags
			Any = SystemBindingFlags.Instance
				| SystemBindingFlags.Static
				| SystemBindingFlags.Public
				| SystemBindingFlags.NonPublic,
			AnyInstance = Any & ~SystemBindingFlags.Static,
			AnyStatic = Any & ~SystemBindingFlags.Instance,
			AnyDeclaredInstance = AnyInstance | SystemBindingFlags.DeclaredOnly,
			AnyDeclaredStatic = AnyStatic | SystemBindingFlags.DeclaredOnly,
			PublicInstance = AnyInstance & ~SystemBindingFlags.NonPublic,
			PublicStatic = AnyStatic & ~SystemBindingFlags.NonPublic,
			PublicDeclaredInstance = AnyDeclaredInstance & ~SystemBindingFlags.NonPublic,
			PublicDeclaredStatic = AnyDeclaredStatic & ~SystemBindingFlags.NonPublic,
			PrivateInstance = AnyInstance & ~SystemBindingFlags.Public,
			PrivateStatic = AnyStatic & ~SystemBindingFlags.Public,
	...
	public static Type? Type(string assembly, string @namespace, string type)
		=> System.Type.GetType($"{@namespace}.{type}, {assembly}");
	...
	public static MethodInfo? GetGenericMethod(this Type type, string name, SystemBindingFlags bindingAttr,
		params Type[] typeArguments)
		=> type.GetMethod(name, bindingAttr)?.MakeGenericMethod(typeArguments);
	
	public static MethodInfo? GetInterfaceMethod(this Type type, Type interfaceType, MethodInfo interfaceMethod)
	{
	...
	public static Type GetElementTypeIfByRef(this Type type) => type.IsByRef ? type.GetElementType()! : type;
	...
	private static ConstructorInfo? TryGetConstructor(Type type, SystemBindingFlags flags,
	...
	public static T? CreateConstructorDelegate<T>(Type type, params Type[] parameters) where T : Delegate
	{
		try
		{
			...
			var needsTempVar = type.IsValueType
				&& parameters.Length == 0
				&& !Array.Exists(type.GetConstructors(AccessTools.allDeclared & ~SystemBindingFlags.Static),
	...
	private static T CreateInstanceUsingActivator<T>(params object?[] args)
		=> (T)Activator.CreateInstance(typeof(T),
			BindingFlags.AnyInstance | SystemBindingFlags.CreateInstance, null,
			args, null, null);
