...
public static class FishTranspiler
{
	...
	public static Container Call(MethodBase method, bool forceNoCallvirt = false)
	{
		...
		return new(forceNoCallvirt || method.IsStatic || (method.DeclaringType?.IsValueType ?? true)
			? OpCodes.Call
	...
	public static void EmitRange(this ILGenerator generator, params Container[] fishTranspilers)
	{
		for (var i = 0; i < fishTranspilers.Length; i++)
			generator.Emit(fishTranspilers[i]);
	}

	public static void EmitRange(this ILGenerator generator, ReadOnlySpan<Container> fishTranspilers)
	{
		for (var i = 0; i < fishTranspilers.Length; i++)
			generator.Emit(fishTranspilers[i]);
	}

	public static void EmitRange(this ILGenerator generator, IEnumerable<Container> fishTranspilers)
	{
		foreach (var instruction in fishTranspilers)
